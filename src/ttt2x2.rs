...
pub fn get_winner(id: &NodeIndex) -> u8 {
	let winner = get_winner_row(id);
	if winner != 0 {
		return winner;
	}
		
	let winner = get_winner_col(id);
	if winner != 0 {
		return winner;
	}
		
	let winner = get_winner_diag(id);
	if winner != 0 {
		return winner;
	}
...
pub fn get_winner_row(id: &NodeIndex) -> u8 {
	let mut id = id.clone();
	let mut team = 0;
...
pub fn get_winner_col(id: &NodeIndex) -> u8 {
...
pub fn get_winner_diag(id: &NodeIndex) -> u8 {
	let mut main_id = id.clone();
	let mut team = 0;
