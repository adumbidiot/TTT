...
impl Compiler {
    ...
    pub fn post_process(&mut self) -> CompilationResult<()> {
        let node_id = self
            .winners
            .pop_front()
            .ok_or(CompilationError::QueueEmpty)?;
    ...
    pub fn export_string_map(&self) -> CompilationResult<HashMap<String, Node>> {
    ...
    pub fn export(&self) -> CompilationResult<NodeMap> {
        Ok(self.get_compilation()?.get_cloned_map())
    }

    pub fn get_nodes_processed(&self) -> CompilationResult<usize> {
        return self
            .get_compilation()
            .map(|compilation| compilation.get_nodes_processed());
    }

    pub fn get_winners_processed(&self) -> CompilationResult<usize> {
        return self
            .get_compilation()
            .map(|compilation| compilation.get_winners_processed());
    }

    pub fn get_nodes_scored(&self) -> CompilationResult<usize> {
        return self
            .get_compilation()
            .map(|compilation| compilation.get_nodes_scored());
    }
...
impl AI {
    ...
    pub fn get_score(&self, id: &NodeIndex) -> Option<i8> {
        return self.get_node(id).map(|node| node.score);
    }
}
